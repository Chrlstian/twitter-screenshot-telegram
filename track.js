...
// // Twitter profile to track
// const TWITTER_USER_HANDLE = 'SuiNetwork';

// // Store the last processed tweet URL
...
//     // Inject CSS to hide login prompts before loading the page
//     await page.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36');
//     await page.setRequestInterception(true);
//     page.on('request', request => request.continue());
...
//         style.textContent = ${JSON.stringify(HIDE_LOGIN_CSS)};
...
//     // Additional CSS injection after page load to ensure elements are hidden
//     await page.addStyleTag({ content: HIDE_LOGIN_CSS });
...
//     // Inject CSS to hide login prompts
//     await page.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36');
//     await page.setRequestInterception(true);
//     page.on('request', request => request.continue());
//     await page.evaluateOnNewDocument(`
...
//     await page.goto(`https://twitter.com/${TWITTER_USER_HANDLE}`, {
//       waitUntil: 'networkidle0',
//       timeout: 60000
//     });

//     // Additional CSS injection after page load
//     await page.addStyleTag({ content: HIDE_LOGIN_CSS });
...
//       // Only process if it's a new tweet
//       if (latestTweet !== lastProcessedTweet) {
//         try {
//           console.log(`Processing latest tweet: ${latestTweet}`);
          
//           // Capture screenshot and get the file path
//           const screenshotPath = await captureTweetScreenshot(latestTweet);
          
//           // Send to Telegram
//           await sendToTelegram(screenshotPath, latestTweet);
          
//           // Update last processed tweet
//           lastProcessedTweet = latestTweet;
          
//           // Clean up screenshot file
//           await fs.unlink(screenshotPath).catch(console.error);
//         } catch (error) {
//           console.error(`Error processing tweet ${latestTweet}:`, error);
//         }
//       } else {
...
// async function sendToTelegram(screenshotPath, tweetUrl) {
//   try {
//     await fs.access(screenshotPath);
//     const screenshot = await fs.readFile(screenshotPath);
//     await bot.sendPhoto(TELEGRAM_CHAT_ID, screenshot, {
//       caption: `Tweet URL: ${tweetUrl}`
//     });
//     console.log('Screenshot sent to Telegram successfully');
//   } catch (error) {
//     console.error('Error sending to Telegram:', error);
